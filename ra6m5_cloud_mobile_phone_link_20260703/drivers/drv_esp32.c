...
static volatile int gEsp32TxBusy = 0;
...
static int esp32_write_byte_wait(uint8_t data);
...
static int esp32_configure_uart_115200(void);
...
static int esp32_write_byte_wait(uint8_t data)
{
    for (int retry = 0; retry < 20; retry++) {
        fsp_err_t err = g_uart2.p_api->write(g_uart2.p_ctrl, &data, 1U);
        if (err == FSP_SUCCESS) {
            esp32_delay_ms(2);
            return ESUCCESS;
        }
...
static int esp32_send_cmd(const char *cmd)
{
    ...
    while (*cmd != '\0') {
        int ret = esp32_write_byte_wait((uint8_t) *cmd);
        if (ret != ESUCCESS) {
            xprintf("[ESP32][UART] write byte err=%d\r\n", ret);
            return ret;
        }
        cmd++;
    }
...
static int esp32_send_data(const uint8_t *data, int len)
{
    ...
    for (int i = 0; i < len; i++) {
        int ret = esp32_write_byte_wait(data[i]);
        if (ret != ESUCCESS) {
            return ret;
        }
    }
...
static void esp32_start_background_rx(void)
{
    ...
    if ((err != FSP_SUCCESS) && (err != FSP_ERR_IN_USE)) {
        xprintf("[ESP32][UART] read start err=%d\r\n", (int) err);
    }
}
...
static int esp32_configure_uart_115200(void)
{
    ...
    err = R_SCI_UART_BaudCalculate(115200U, false, 5000U, &baud_setting);
    if (err != FSP_SUCCESS) {
        xprintf("[ESP32][UART] baud calc err=%d\r\n", (int) err);
        return -EIO;
    }

    err = g_uart2.p_api->baudSet(g_uart2.p_ctrl, &baud_setting);
    if (err != FSP_SUCCESS) {
        xprintf("[ESP32][UART] baud set err=%d\r\n", (int) err);
        return -EIO;
    }

    xprintf("[ESP32][UART] baud=115200 brr=%u bgdm=%u abcs=%u abcse=%u cks=%u\r\n",
            baud_setting.brr,
            baud_setting.semr_baudrate_bits_b.bgdm,
            baud_setting.semr_baudrate_bits_b.abcs,
            baud_setting.semr_baudrate_bits_b.abcse,
            baud_setting.cks);
...
int esp32_init(void)
{
    ...
    if (esp32_configure_uart_115200() != ESUCCESS) {
        return -EIO;
    }
    esp32_start_background_rx();
    esp32_flush_rx();
...
int esp32_diag_at(char *resp_buf, int buf_len)
{
    ...
    if (resp_buf && (buf_len > 0)) {
        memset(resp_buf, 0, (size_t) buf_len);
    }
    ...
    esp32_flush_rx();
    xprintf("[ESP32][DIAG] send AT\r\n");

    if (esp32_send_cmd("AT\r\n") != ESUCCESS) {
        return -EIO;
    }

    rx_len = esp32_wait_response(TIMEOUT_AT_BASIC);
    if ((resp_buf != NULL) && (buf_len > 0) && (rx_len > 0)) {
        strncpy(resp_buf, (const char *) gEsp32RxBuf, (size_t) buf_len - 1U);
        resp_buf[buf_len - 1] = '\0';
    }

    if (rx_len <= 0) {
        xprintf("[ESP32][DIAG] no response\r\n");
        return -ETIMEDOUT;
    }

    xprintf("[ESP32][DIAG] resp=%s\r\n", gEsp32RxBuf);
    return esp32_response_ok();
}
